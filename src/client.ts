...
export interface ClientOptions {
   ...
   *
   * Defaults to process.env['CHECKOUT_INTENTS_BASE_URL'].
   */
   ...
   * Set the log level.
   *
   * Defaults to process.env['CHECKOUT_INTENTS_LOG'] or 'warn' if it isn't set.
   */
...
export class CheckoutIntents {
   ...
   */
  constructor({
    baseURL = readEnv('CHECKOUT_INTENTS_BASE_URL'),
    apiKey = readEnv('CHECKOUT_INTENTS_API_KEY'),
    ...opts
  }: ClientOptions = {}) {
    if (apiKey === undefined) {
