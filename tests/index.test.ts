...
import { APIPromise } from 'checkout-intents/core/api-promise';

import util from 'node:util';
import CheckoutIntents from 'checkout-intents';
import { APIUserAbortError } from 'checkout-intents';
...
describe('logging', () => {
  ...
  beforeEach(() => {
    process.env = { ...env };
    process.env['CHECKOUT_INTENTS_LOG'] = undefined;
  });
  ...
  const forceAPIResponseForClient = async (client: CheckoutIntents) => {
  ...
  test('debug logs happen with debug env var', async () => {
    ...
    process.env['CHECKOUT_INTENTS_LOG'] = 'debug';
    const client = new CheckoutIntents({ logger: logger, apiKey: 'My API Key' });
  ...
  test('warn when env var level is invalid', async () => {
    ...
    process.env['CHECKOUT_INTENTS_LOG'] = 'not a log level';
    const client = new CheckoutIntents({ logger: logger, apiKey: 'My API Key' });
    expect(client.logLevel).toBe('warn');
    expect(warnMock).toHaveBeenCalledWith('process.env[\'CHECKOUT_INTENTS_LOG\'] was set to "not a log level", expected one of ["off","error","warn","info","debug"]');
  });
  ...
  describe('baseUrl', () => {
    test('trailing slash', () => {
      const client = new CheckoutIntents({ baseURL: 'http://localhost:5000/custom/path/', apiKey: 'My API Key' });
    ...
    afterEach(() => {
      process.env['CHECKOUT_INTENTS_BASE_URL'] = undefined;
    });

    test('explicit option', () => {
      const client = new CheckoutIntents({ baseURL: 'https://example.com', apiKey: 'My API Key' });
    ...
    test('in request options overridden by client options', () => {
      const client = new CheckoutIntents({ apiKey: 'My API Key', baseURL: 'http://localhost:5000/client' });
      expect(client.buildURL('/foo', null, 'http://localhost:5000/option')).toEqual('http://localhost:5000/client/foo');
    });

    test('in request options overridden by env variable', () => {
      process.env['CHECKOUT_INTENTS_BASE_URL'] = 'http://localhost:5000/env';
      const client = new CheckoutIntents({ apiKey: 'My API Key' });
  ...
  test('maxRetries option is correctly set', () => {
    const client = new CheckoutIntents({ maxRetries: 4, apiKey: 'My API Key' });
    expect(client.maxRetries).toEqual(4);

    // default
    const client2 = new CheckoutIntents({ apiKey: 'My API Key' });
  ...
  test('with environment variable arguments', () => {
    ...
    process.env['CHECKOUT_INTENTS_API_KEY'] = 'My API Key';
    const client = new CheckoutIntents();
  ...
  test('with overridden environment variable arguments', () => {
    // set options via env var
    process.env['CHECKOUT_INTENTS_API_KEY'] = 'another My API Key';
    const client = new CheckoutIntents({ apiKey: 'My API Key' });
    expect(client.apiKey).toBe('My API Key');
  });
});
...
describe('idempotency', () => {
  test.skip('key can be set per-request', async () => {
    const client = new CheckoutIntents({ baseURL: process.env["TEST_API_BASE_URL"] ?? 'http://127.0.0.1:4010', apiKey: 'My API Key' });
...
describe('retries', () => {
  ...
  test('retry on 429 with retry-after', async () => {
    ...
    const client = new CheckoutIntents({ apiKey: 'My API Key', fetch: testFetch });
...
describe('environment auto-inference', () => {
  // Use origin (scheme + host) to match the environment URL without being
  // sensitive to trailing slash differences in the OpenAPI spec.
  const baseOrigin = (url: string) => new URL(url).origin;
  ...
  test('baseURL overrides auto-inferred environment', () => {
    ...
    expect(baseOrigin(client.baseURL)).toEqual('https://custom.api.example.com');
  });
